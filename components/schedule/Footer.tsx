...
export function Footer({ currentLang, title, reqs, onTitleUpdate, onReqUpdate }: FooterProps) {
  ...
  return (
    <div id="schedule-footer" className="mt-4 md:mt-6 bg-white p-3 md:p-6 rounded-lg shadow-sm border border-slate-200 text-xs md:text-base print:mt-0 print:border-none print:shadow-none print:text-[8px] print:p-1 print:leading-tight">
      <div className="flex flex-col md:flex-row gap-2 md:gap-4 print:gap-1">
        <div className="font-bold text-gray-900 whitespace-nowrap min-w-[100px] md:min-w-[120px] print:min-w-[80px]">
          <input
            type="text"
            value={title}
        ...
        <ul className="text-gray-800 space-y-1 md:space-y-2 list-none w-full print:space-y-0">
          {reqs.map((req, idx) => (
